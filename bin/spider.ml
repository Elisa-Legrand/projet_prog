(**open Utils open Effect open Effect.Deep open Engine

   let spider_egg_cooldown = 20 (* en nombre de tours *) let spider_egg_lifetime
   = 60

   let spawn_egg_nearby position = let pos_egg = get_random_empty_adjacent_cell
   in

   let spawn_spider_nearby position = let pos_baby =
   get_random_empty_adjacent_cell in
   ...
   let rec spider (current_position :int * int) : unit = move_dir
   current_position random_dir; let spawn_egg_bool = (Random.int 100 = 0) in if
   spawn_egg_bool then spawn_egg_nearby; render (); if safe_perform(id)then
