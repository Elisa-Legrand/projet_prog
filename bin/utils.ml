...
let get_walkable_adjacent_cells (crea : creature) ((x, y) : int * int) : (int * int) list =
  List.filter
    (fun (x, y) -> let target_crea = get_content (x, y) in
    target_crea = Empty || can_stomp crea target_crea)
    (get_adjacent_cells (x, y))

(** [a_star crea src dest] renvoie une liste de cases successives, si elle existe,
    que [crea] doit parcourir pour atteindre [dest] à partir de [src] en utilisant 
    l'algorithme A*, munie de la distance de Manhattan comme heuristique.
    La fonction enclenche [No_path_found] si il n'y a pas de chemin de [src] à [dist].
    La donnée de [crea] permet de savoir sur quelle case on peut marcher. *)
let a_star (crea : creature) (src : int * int) (dest : int * int) : (int * int) list = 
  ...
  let is_explored ((x, y) : int * int) : bool =
    get_dist_from_src (x, y) <> max_int in

  let priority ((x, y) : int * int) : int =
    get_dist_from_src (x, y) + (manhattan_distance (x, y) dest) in

  let rec treat_neighbors (curr : int * int) (neighbors : (int * int) list) =
    match neighbors with
    | [] -> ()
    | (neighbor)::q -> if not (is_explored neighbor) then
  ...
  let rec _aux_reconstruct_path cell = 
    if cell = src then [src] else cell::(_aux_reconstruct_path (Hashtbl.find parent cell))
  in
