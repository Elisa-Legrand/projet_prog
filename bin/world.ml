...
let get (x, y : int * int) : cell = try world.(x).(y) with Invalid_argument _ -> (Invalid,invalid_id)
...
let get_id (x, y : int * int) : int =
    ...
    if content = Empty && id <> invalid_id then failwith "Empty cell with wrong id detected" else id
