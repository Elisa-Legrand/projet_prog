...
let pqueue_mem pq elt =
  List.exists (fun (_, e) -> e = elt) pq.l
...
(** [_pqueue_list_find_min l] donne le couple de priorité minimale le plus
    à gauche dans [l] *)
let rec _pqueue_list_find_min (l : (int * 'a) list) : int * 'a = match l with
...
let pqueue_pop pq = 
  if pq.size = 0 then raise Empty_priority_queue else
